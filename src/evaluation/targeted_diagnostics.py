...
All paraphrase is deterministic: temperature 0.3 / top-k 40 / fixed seed.
"""
...
class TargetedDiagnostics:
    ...
    def run(self):
        torch.manual_seed(0)
        print("=" * 70)
        print("TARGETED DIAGNOSTICS:", self.checkpoint_path)
        ...
        print("\n" + "=" * 70)
        print("TARGETED DIAGNOSTICS SUMMARY")
        print("=" * 70)
        print(f"  A (semantic, n={cat_a_counts['n']}): "
              f"strict {cat_a_strict*100:5.1f}%  loose {cat_a_loose*100:5.1f}%")
        print(f"  B (paraphrase): "
...
def main():
    ...
    bench = TargetedDiagnostics(args.checkpoint)
    result = bench.run()
