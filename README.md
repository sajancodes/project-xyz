...
## REAL TEST — LATEST BEST MODEL (`checkpoint_best.pt`)

**Score: 5/15 (33%)** — live generations from the actual checkpoint (step 3000, NOT the step-20 smoke test).

### Test Configuration
| Setting | Value |
|---------|-------|
| Checkpoint | `checkpoints/best/checkpoint_best.pt` |
| Training step | 3000 |
| Training loss | 3.5710 |
| Training type | `semantic_v2_mixed` |
| Base checkpoint | `checkpoints/pretrain/checkpoint-106k.pt` |
| Parameters | 14,657,664 (14.66M) |
| Device | cpu |
| Temperature | 0.3 |
| Top-K | 40 |
| max_new_tokens | 40 |
| Chat wrapper | ON (`User: ...\nAssistant:`) |
...
### Analysis
- **Can do**: single-fact object-property retrieval (tests 1, 2), sun-rises-east (9), simple forward relations (6, 13).
- **Cannot do**: multi-fact reasoning/disambiguation (4, 5, 8, 14, 15 — answers with random prompt snippet or hallucination), paraphrase (7 — color wrong), sunset/west (10), negation inference (12 — repeats prompt), instruction content (11 — correct format, wrong answer).
- **Grammar stability**: degraded output like `Sajan is a red car.` / `Yes, Jenny is a dog barks.`
- **vs composite 0.7475**: benchmark scores category strengths (relations/why_context/transitive 100%); these ad-hoc prompts stress known weak categories (entity_tracking 60%, sun_facts 17% with set→west 12.5%, instruction phrasing-sensitive).

---

*Last updated: August 13, 2026 (Real test of latest best model `checkpoint_best.pt` completed: 5/15 PASS; single-fact retrieval works, multi-fact/paraphrase/sunset/negation remain gaps)*
